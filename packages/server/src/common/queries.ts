...
// TB_SQL_HIST
// export const querySqlStat = `SELECT H.SNAP_ID, H.DBID,H.INSTANCE_NUMBER,SQL_ID,
...
// `;
export const querySqlStat = `
SELECT SS.* 
FROM DBA_HIST_SQLSTAT SS, DBA_HIST_SNAPSHOT SH
WHERE SS.SNAP_ID = SH.SNAP_ID
AND SH.END_INTERVAL_TIME > SYSDATE - 1
`;
// WHERE SNAP_ID >= (SELECT MAX(snap_id) from dba_hist_snapshot)`;

// TB_SQL_TEXT

export const querySqlText = `
SELECT DBID, SQL_ID, SUBSTR(LTRIM(SQL_TEXT), 0, 1000) AS SQL_TEXT, COMMAND_TYPE FROM DBA_HIST_SQLTEXT
`;
// S
...
SELECT SNAP_ID, DBID, INSTANCE_NUMBER, STARTUP_TIME, BEGIN_INTERVAL_TIME, END_INTERVAL_TIME 
